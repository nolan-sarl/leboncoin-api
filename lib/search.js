...
Search.prototype.run = function (proxy = null, isSession = true, options, bodyParams) {
    var self = this;
    if (options == null) {
        options = {}
    }
    if (options.session == null || !isSession) {
        options.session = utils.defaultSession;
    }
    bodyParams = (bodyParams) ? bodyParams : this.getBodyParams();

    let headers = options.session.getHeader()
            ...
            request({
                ...
                if (err && err.code == "ECONNRESET") {
                    ...
                    if (
                        ...
                        resolve({
                            success: false,
                            code: httpResponse ? httpResponse.statusCode : null
                        });
                    } else {
